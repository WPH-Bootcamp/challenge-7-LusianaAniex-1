...
const LoadingState = ({ title = 'My Cart', message = 'Loading cart...' }: LoadingStateProps) => (
...
const QuantityControls = ({ quantity, onDecrease, onIncrease, isMobile = false }: QuantityControlsProps) => {
...
const CartItemCard = ({ item, onQuantityChange, isMobile = false }: CartItemCardProps) => {
  ...
  const RestaurantHeader = () => (
    <div
      className={`flex flex-row items-center gap-2 w-full ${
        isMobile ? 'h-8 cursor-pointer' : 'mb-5 cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors'
      }`}
  ...
  const groupedItems = useMemo(() => 
    cartItems.reduce((acc, item) => {
      ...
      if (!acc[restaurantId]) {
        acc[restaurantId] = {
          restaurantId,
          restaurantName: restaurant?.name || item.restaurantName || 'Restaurant Name',
